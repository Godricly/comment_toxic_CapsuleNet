...
def net_define_eu():
    ...
    with net.name_scope():
        ...
        net.add(transpose(axes=(0,2,1)))
        net.add(nn.MaxPool1D(pool_size=2))
        net.add(transpose(axes=(0,2,1)))
        '''
        net.add(rnn.GRU(128,layout='NTC',bidirectional=True, num_layers=1, dropout=0.2))
        net.add(transpose(axes=(0,2,1)))
        ...
        net.add(CapFullyNGBlock(16, num_cap=32, input_units=32, units=16, route_num=3))
        net.add(nn.Dropout(0.2))
        net.add(nn.Dense(12*16, activation='relu'))
        net.add(nn.Dropout(0.2))
        net.add(nn.Dense(6, activation='sigmoid'))
    net.initialize(init=init.Xavier())
    return net
...
class FeatureBlock1(nn.Block):
    def __init__(self, **kwargs):
        ...
        self.gru = rnn.GRU(128,layout='NTC',bidirectional=True, num_layers=1, dropout=0.2)
        # self.conv3 = nn.Conv1D(channels=64, kernel_size=5, padding=2, strides=2, activation='relu')
        # self.conv5 = nn.Conv1D(channels=64, kernel_size=9, padding=4, strides=2, activation='relu')
        # self.conv7 = nn.Conv1D(channels=64, kernel_size=13, padding=6, strides=2, activation='relu')
    ...
    def forward(self, x):
        ...
        # conv_pooled = nd.concat(*[conv_max_pooled, conv_ave_pooled], dim=1)
        # conv_dropped = self.conv_drop(conv_pooled)
        # conv_dropped = self.conv_drop(conv_out)
        # concated_feature = nd.concat(*[gru_maxpooled, conv_dropped], dim=1)
        # return concated_feature
